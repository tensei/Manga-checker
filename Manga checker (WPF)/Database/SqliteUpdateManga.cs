    ...
    public class SqliteUpdateManga {
        public SqliteUpdateManga(MangaModel manga,
            ...
            try {
                ...
                if (!manga.Site.ToLower().Equals("backlog") && linkcol) {
                    new SQLiteCommand(
                        $@"INSERT INTO link_collection (name, chapter, added, link, site) VALUES ('{manga.Name}', '{manga
                            .Chapter
                            }', (datetime()), '{manga.Link}', '{manga.Site
                                .ToLower()}')", mDbConnection).ExecuteNonQuery();
                    addToNew(manga);
                    new SQLiteCommand(
                        $@"INSERT INTO mangasnotread (name, chapter, last_update, link, site) VALUES ('{manga.Name}', '{manga
                            .Chapter
                            }', (datetime()), '{manga.Link}', '{manga.Site
                                .ToLower()}')", mDbConnection).ExecuteNonQuery();
                }
        ...
        private void addToNew(MangaModel manga) {
            Application.Current.Dispatcher.BeginInvoke(
                new Action(() => { GlobalVariables.NewMangasInternal.Add(manga); }));
        }
    }
}