...
namespace Manga_checker.Utilities {
    static class ImageLinkCollecter {
        ...
        public static List<string> YomangaCollectLinks(string url) {
            ...
            for (int i = 1; i <= lastChapterNumber; i++) {
                ...
                var htmlimg = GetSource.Get(newlink.Replace("http:/yo", "http://yo")) ?? CloudflareGetString.Get(newlink.Replace("http:/yo", "http://yo"));

                var imgLink = Regex.Match(htmlimg, "<img class=\"open\" src=\"(http://yomanga.co/reader/content/comics.+)\".+");
        ...
        public static List<string> MangastreamCollectLinks(string url) {
            ...
            for (int i = 1; i <= lastChapterNumber; i++) {
                ...
                var htmlimg = GetSource.Get(newlink.Replace("http:/man", "http://man")) ?? CloudflareGetString.Get(newlink.Replace("http:/man", "http://man"));
