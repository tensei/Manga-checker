...
namespace MangaChecker.Models {
	...
	public class MangaModel {
		public MangaModel() {
			...
			DoubleClickCommand = new ActionCommand(() => { Process.Start(Link); });
		}
		...
		private Visibility SetNewVisibility() {
			if (GlobalVariables.ViewerEnabled.Contains(Site.ToLower()) && (Link != "placeholder")) return Visibility.Visible;
		...
		private List<Button> PopulateButtons() {
			...
			if (gg.Contains(Site.ToLower()))
				for (var i = 0; i < 3; i++) {
					if (Chapter.Contains(" ")) Chapter = Chapter.Split(new[] {" "}, StringSplitOptions.RemoveEmptyEntries)[0];
			...
			else Separator = Visibility.Collapsed;
			return list;
		}
